...
class Server extends OperatorResource implements Creatable, Updateable, Deletable, Retrievable, Listable
{
    ...
    /**
     * huyuxi 2019-12-17
     * add force-delete servers api
     */
    public function deleteForce()
    {
        $this->execute($this->api->deleteForceServer(),[
            'id'       => $this->id,
            'forceDelete' => ['force' => "null"],
        ]);
    }
    ...
    //huyuxi 2019-12-26 get a server detail
    public function getServerInfo()
    {
        $response = $this->execute($this->api->getServer(), ['id' => $this->id]);
        return $this->model(Server::class)->populateFromResponse($response);
    }
}
